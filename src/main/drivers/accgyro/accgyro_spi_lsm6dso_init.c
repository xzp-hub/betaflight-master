...
#define LSM6DS3TR_C_CHIP_ID 0x6A  // LSM6DS3TR-C chip ID
...
uint8_t lsm6dsoDetect(const extDevice_t *dev)
{
    ...
    if (busReadRegisterBuffer(dev, LSM6DSO_REG_WHO_AM_I, &chipID, 1)) {
        if (chipID == LSM6DSO_CHIP_ID || chipID == LSM6DS3TR_C_CHIP_ID) {
            return LSM6DSO_SPI;
        }
    }
...
static void lsm6dsoConfig(gyroDev_t *gyro)
{
    ...
    // LSM6DS3TR-C specific initialization (if chip ID is 0x6A)
    uint8_t chipID = 0;
    if (busReadRegisterBuffer(dev, LSM6DSO_REG_WHO_AM_I, &chipID, 1) && chipID == LSM6DS3TR_C_CHIP_ID) {
        // LSM6DS3TR-C requires additional configuration for proper operation
        // Configure MD1_CFG for data ready interrupts
        lsm6dsoWriteRegister(dev, 0x5E, 0x40, 1);  // MD1_CFG: gyro data ready on INT1
    }
}
