...
bool mpuAccReadSPI(accDev_t *acc)
{
    ...
    case GYRO_EXTI_INT_DMA:
    {
        ...
        if (acc->gyro->mpuDetectionResult.sensor == ICM_42670_SPI) {
            // For ICM-42670-L: data layout with rxBuf[0] aligned
            // rxBuf[0-1] = dummy (command response)
            // rxBuf[2-3] = Accel X, rxBuf[4-5] = Accel Y, rxBuf[6-7] = Accel Z
            // int16_t index: [0]=dummy, [1]=AccelX, [2]=AccelY, [3]=AccelZ
            acc->ADCRaw[X] = __builtin_bswap16(accData[1]);
            acc->ADCRaw[Y] = __builtin_bswap16(accData[2]);
            acc->ADCRaw[Z] = __builtin_bswap16(accData[3]);
        } else {
            // For other chips: data starts at rxBuf[1] (odd alignment)
            // int16_t index: [0]=rxBuf[0-1], [1]=rxBuf[2-3], etc.
            acc->ADCRaw[X] = __builtin_bswap16(accData[1]);
            acc->ADCRaw[Y] = __builtin_bswap16(accData[2]);
            acc->ADCRaw[Z] = __builtin_bswap16(accData[3]);
        }
...
bool mpuGyroReadSPI(gyroDev_t *gyro)
{
    ...
    case GYRO_EXTI_INIT:
    {
        ...
        if (gyro->detectedEXTI > GYRO_EXTI_DETECT_THRESHOLD) {
            ...
            if (spiUseDMA(&gyro->dev)) {
                ...
                // ICM-42670-L has continuous data registers, calculate correct length
                if (gyro->mpuDetectionResult.sensor == ICM_42670_SPI) {
                    // For ICM-42670-L: accDataReg=0x0B, gyroDataReg=0x11
                    // Need to read: 0x0B-0x10 (accel 6 bytes) + 0x11-0x16 (gyro 6 bytes) = 12 bytes total
                    // SPI read: 1 command byte + 12 data bytes = 13 bytes total
                    // Using rxBuf[0] as start for proper int16_t alignment
                    gyro->segments[0].len = 12 + 1;  // 12 data bytes + 1 command byte
                } else {
                    // For other ICM426xx chips
    ...
    case GYRO_EXTI_INT_DMA:
    {
        ...
        uint8_t gyroDataIndex;
        
        if (gyro->mpuDetectionResult.sensor == ICM_42670_SPI) {
            // For ICM-42670-L: data layout with rxBuf[0] aligned
            // rxBuf[0-1] = dummy (command response)
            // rxBuf[2-7] = Accel X/Y/Z, rxBuf[8-13] = Gyro X/Y/Z
            // int16_t index: [0]=dummy, [1]=AccelX, [2]=AccelY, [3]=AccelZ, [4]=GyroX, [5]=GyroY, [6]=GyroZ
            gyroDataIndex = 4;  // Gyro data starts at index 4
        } else {
            // For other ICM426xx chips: data starts at rxBuf[1]
            gyroDataIndex = ((gyro->gyroDataReg - gyro->accDataReg) >> 1) + 1;
        }
